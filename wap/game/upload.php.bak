...
if($user) {
	if(isset($_POST['submit'])) {
		...
		// формат файла
		$uploaddir = $_SERVER['DOCUMENT_ROOT'].'/wap-down/';
		$mds = $con->query("SELECT * FROM `package`  ORDER BY `id` DESC LIMIT 1")->fetch_assoc();
        $rand = $mds['id']+1;
		...
		$fid = $con->query("SELECT * FROM `soft`  ORDER BY `id` DESC LIMIT 1")->fetch_assoc();
		$fid = $fid['id']+1;
		//$f = $fid['id'];
		    //$id = filtr($_POST['id']);
			$raz = filtr($_POST['raz']);
...
//foreach ($list as $i => $file) {
    $ftext = readZipText("../wap-down/".$rand."".$f."". $file, "META-INF/MANIFEST.MF"); 
    $ticon = readZipText("../wap-down/".$rand."".$f."". $file, getJarIconName($ftext));
...
$size = getFilesizes($_SERVER['DOCUMENT_ROOT'].'/wap-down/'.$rand.''.$f.'');
$names = $jar_name ?: $mrp_name;
$vendors = $jar_vendor ?: $mrp_vendor;
$versions = $jar_version ?: $mrp_version;
$text2 = $jar_text ?: $jar_name;
    $upload = $con->query("SELECT name,author,dpi,size FROM `package` WHERE `name` = '".$names."' AND `author` = '".$vendors."' AND `size` = '".$size."'")->fetch_assoc();
...
$file_path='../jar/'.$fid.'/' . getJarIconName($ftext) . '';
$to_file_path='../icon.baorua.cn/'.$fid.'.png';
...
if ($names){
$con->query("INSERT INTO `package` (`id`, `raz`, `id_user`, `icon`, `name`, `author`, `platform`, `down`, `zh`, `size`, `DJ`, `dpi`, `text`, `v`, `system`, `time`, `format`) VALUES (NULL, '".$raz."', '".$user['id']."', '".$fid.".png', '".$names."', '".$vendors."', '".$platform."', '".$rand.$f."', '".$zh."', '".$size."', '".$DJ."', '".$dpi."', '".$text2."', '".$versions."', 'game', '".time()."', '".$f."')");
}
}
//if ($con->query("INSERT INTO `games` (`id`, `id_game`, `id_user`, `down`, `zh`, `vv`, `DJ`, `pay`, `dpi`, `time`, `format`) VALUES (NULL, '$up', '{$user['id']}', '$rand$f', '{$zh}', '{$vv}', '{$DJ}',  '{$pay}', '{$dpi}', '".time()."', '{$f}')"); === TRUE) {
    //echo "新记录插入成功";
...
$bx = $con->query("SELECT * FROM `package` WHERE `id` = '".$fid."'")->fetch_assoc();
if($bx['id'] > 0){
$title = '消息提示';
echo '<body id="notice"><h2 class="topic">消息提示</h2><p>安装成功。</p><p><a href="//wap.baorua.cn/game/'.$fid.'.html" class="button">返回</a></p></body>';
exit();
}
...
//$b = $con->query("SELECT * FROM `class`");
	...
	echo '<div id="aside"><h2 class="topic">注意事项</h2><ul class="list list1"><li>1、请选择正确的分辨率和游戏类型</li><li>2、目前仅可上传MRP,JAR格式的手机游戏</li><li>3、SIS和Ngage格式<a href="/"><em>点击这里</em></a></li><li>4、最大上传容量10MB</li></ul></div></main>';
