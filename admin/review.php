...
$k_post = $con->query('SELECT * FROM `games` WHERE `id_game` = 0')->num_rows;
	
//$k_page = k_page($k_post,12);
	
		$ms = $con->query("SELECT * FROM `games` WHERE `id` AND `id_game` = 0 ORDER BY `id` DESC LIMIT 12");
...
echo '<ul class="list comment line padding bg-white" id="comment"><li class="clearfix"><a href="/info/16835"><img src="/M/ii/'.$w['id'].'" width="46" height="46" alt="头像" /></a><div><p><a href="/packages/edit/'.$w['id'].'">system: ' . getJarIniName($ftext, "MIDlet-Name") . '</a></p>';
echo '<div class="small-font"><a class="right">';
if($w['id_game']==0) echo '审核通过'; else echo '不通过';
echo '</a></div></div></li>';
