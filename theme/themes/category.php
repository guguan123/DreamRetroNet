...
include_once $_SERVER["DOCUMENT_ROOT"] . '/M/c/user.php';
	...
	//名字
	$key = filtr($_GET['category']);
$k_post = $con->query("SELECT * FROM `game` WHERE `id_raz` LIKE '%$key%'")->num_rows;
...
$start = 20*$page-20;
		$ms = $con->query("SELECT * FROM `game` WHERE `id_raz` LIKE '%$key%' order by id DESC LIMIT $start, 20");
		//$ms = $con->query("SELECT * FROM `file` WHERE `author` = '".$author."' ORDER BY `id` DESC LIMIT 10");

if($k_post  < 1) err('没有同类游戏！');
...
echo '<li><a href="/game/'.$w['id'].'" class="clearfix"><img src="/M/i/'.$w['id'].'" width="46" height="46" alt="图标" />';
if($name = $w['cn'] ?: $w['name']){
echo '<strong>'.$name.'</strong>';
}
echo '<span class="small-font">'.$w['platform'].'&nbsp;'.$w['downs'].'下载&nbsp;';
$number = $con->query('SELECT * FROM `comment` WHERE `id_obmen` = "'.$id.'"')->num_rows;
echo ''.$number.'评论&nbsp;'.$w['id_raz'].'</span></a></li>';
...
if($k_post > '20') {  echo str('?category='.$category.'&',$k_page,$page.'');  }
//}
