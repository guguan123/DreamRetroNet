...
$con->query("UPDATE `file` SET `text` = '".$text."', `id_raz` = '".$id_raz."', `author` = '".$author."', `name` = '".$name."', `zhcn` = '".$zhcn."', `platform` = '".$platform."' WHERE `id` = '".$id."'");
if($user['admin_level']>=3){
$con->query("UPDATE `file` SET `id_user` = '".$id_user."', `rek` = '".$rek."', `ivent` = '".$ivent."' WHERE `id` = '".$id."'");
}
header('Location: /file/'.$b['id']);
}
}
echo '<h2 class="topic">编辑应用资料</h2>
...
echo '
<label>应用名：</label>
<input type="text" name="name" value="' . getJarIniName($ftext, "MIDlet-Name") . '" required="required" >
</div><div>
<label>中文名：</label>
<input type="text" name="zhcn" value="'.$b['zhcn'].'" /></div><div>
<label>平台：</label>
<select name="platform" required="required">
<option value="'.$b['platform'].'">'.platform($b['platform']).'</option>
...
<label>提供商：</label>
<input type="text" name="author" value="' . getJarIniName($ftext, "MIDlet-Vendor") . '" required="required" >
</div><div>
<label>类型：</label>
...
echo '<li><a href="/download/'.$x['id'].'" title="下载">'.dpi($x['dpi']).'</a><div>'.zh($x['zh']).' | '.pay($x['pay']).' | '.DJ($x['DJ']).' | '.vv($x['vv']).' | <a href="/file_jar/'.$x['id'].'">修改</a></div></li>';
}
}
