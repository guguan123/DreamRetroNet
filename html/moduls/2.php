...
$files = $con->query("SELECT * FROM `file` WHERE `id` ORDER BY `id` DESC");
while($w = $files->fetch_assoc()){

$list = get_file_folder_List("jar", 2, '*');
//foreach ($list as $i => $file) {
    $bitmap = base64_encode(readZipText("../down/files/".$w['down']."" . $file, "icon.png"));
    echo sprintf('<img src="data:image/%s;base64,%s" style="width:64px;height:64px;"><br/>', "png", $bitmap);
    $ftext = readZipText("../down/files/".$w["down"]."" . $file, "META-INF/MANIFEST.MF"); 
    echo "游戏名：" . getJarIniName($ftext, "MIDlet-Name") . "<br/>";
    echo "简   介：" . getJarIniName($ftext, "MIDlet-Description") . "<br/>";
    echo '下   载：<a href="./jar/' . gbk2utf8($file) . '">' . gbk2utf8($file) . '</a><br/><br/>';
    echo "'".$w['down']."";
   } 
//}