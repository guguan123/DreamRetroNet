...
if($user) {
	if(isset($_POST['submit'])) {
		...
		// формат файла
		$uploaddir = $_SERVER['DOCUMENT_ROOT'].'/api-down/';
...
//foreach ($list as $i => $file) {
    $ftext = readZipText("../../api-down/".$rand."".$f."". $file, "META-INF/MANIFEST.MF"); 
...
$size = getFilesizes($_SERVER['DOCUMENT_ROOT'].'/api-down/'.$rand.''.$f.'');
...
if ($names){
$con->query("INSERT INTO `package` (`id`, `raz`, `id_user`, `name`, `author`, `platform`, `down`, `zh`, `size`, `DJ`, `dpi`, `text`, `v`, `system`, `time`, `format`) VALUES (NULL, '".$raz."', '".$user['id']."', '".$names."', '".$vendors."', '".$platform."', '".$rand.$f."', '".$zh."', '".$size."', '".$DJ."', '".$dpi."', '".$text2."', '".$versions."', 'game', '".time()."', '".$f."')");
}
}
//if ($con->query("INSERT INTO `games` (`id`, `id_game`, `id_user`, `down`, `zh`, `vv`, `DJ`, `pay`, `dpi`, `time`, `format`) VALUES (NULL, '$up', '{$user['id']}', '$rand$f', '{$zh}', '{$vv}', '{$DJ}',  '{$pay}', '{$dpi}', '".time()."', '{$f}')"); === TRUE) {
    //echo "新记录插入成功";
