...
$keys = $con->query(" SELECT * FROM `search_theme_log` WHERE num > 0 ORDER BY RAND() LIMIT 12");
...
//</form></div>';	
	...
	$keyword = filtr($_GET['keyword']);
	 $key_num = $con->query(" SELECT * FROM `theme_search_log` WHERE `key` = '$keyword'")->num_rows;
    if($key_num > 0){
        $con->query("UPDATE `theme_search_log` SET `num` = `num`+1 WHERE `key` = '$keyword'");
        
    }else{
        $con->query("INSERT INTO `theme_search_log` (`id`, `key`, `num`, `time`) VALUES (NULL, '{$key}', '1', '')");
        
    }
$k_post = $con->query("SELECT * FROM `theme` WHERE `name` LIKE '%$keyword%'")->num_rows;
...
$start = 12*$page-12;
		$ms = $con->query("SELECT * FROM `theme` WHERE `name` LIKE '%$keyword%' order by id DESC LIMIT $start, 12");
...
echo '<li><a href="/theme/'.$w['id'].'"><img src="/M/i/'.$w['icon'].'" width="24" height="24" alt="图标" />'.$w['name'].'</a></li>';
}
}
echo '</ul>';
