...
//foreach ($list as $i => $file) {
    ...
    $jar_version = trim($file_version);
     $rowArr = json_encode(array("id" => "".$row['id']."","nickname" => "".$users['name']."","size" => "".$size."","version" => "".$jar_version."","lang" => "".$row['zh']."","resolution" => "".$row['dpi']."","online" => "".$row['DJ']."","format" => "".$row['format']."","users_id" => "".$row['id_user']."","avatar" => "".avatar1($row['id_user']).""));
    $str=$str.$rowArr."###";
}
$jsonArr=rtrim($str,"###");
