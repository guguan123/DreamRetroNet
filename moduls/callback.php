...
//var_dump($user_num);
    if ($user_num > 0){
    $con->query("UPDATE `user` SET `name` = '".$_SESSION['nickname']."', `baidu_key` = '".$_SESSION['accesstoken']."', `baidu_pic` = '".$_SESSION['smallpic']."' WHERE `baidu_id` = '$baidu_id'");
     setcookie('baidu_id', $baidu_id, time()+86400*365, '/');  
    }else{
      $con->query("INSERT INTO `user` (`id`, `baidu_id`, `name`, `baidu_pic`, `baidu_key`, `data_reg`) VALUES (NULL, '{$_SESSION['baidu_id']}', '{$_SESSION['nickname']}', '{$_SESSION['smallpic']}', '{$_SESSION['accesstoken']}', '".time()."')");
