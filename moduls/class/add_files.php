...
$x = $con->query("SELECT * FROM `files` WHERE `id_file` = '".$b['id']."'")->fetch_assoc();
...
if($user) {
	if(isset($_POST['submit'])) {
		...
		if (move_uploaded_file($_FILES['userfile']['tmp_name'], $uploadfile)) {
			...
			//介绍
			$con->query("INSERT INTO `files` (`id`, `id_file`, `id_user`, `down`, `zh`, `vv`, `DJ`, `pay`, `dpi`, `time`, `format`, `ivent`) VALUES (NULL, '$fid', '{$user['id']}', '$rand$f', '{$zh}', '{$v}', '{$vv}', '{$DJ}',  '{$pay}', '{$dpi}', '".time()."', '{$f}', '0')");
