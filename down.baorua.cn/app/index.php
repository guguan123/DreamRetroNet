...
$con->query("UPDATE `package` SET `downs` = `downs`+'1' WHERE `id` = '".$b['id']."'");
if ($user['id']){
$con->query("UPDATE `user` SET `package_downs` = `package_downs`+'1' WHERE `id` = '".$user['id']."'");

$down = $con->query(" SELECT * FROM `package_download` WHERE `user_id` = '".$user['id']."' AND `package_id` = '".$b['id']."'")->num_rows;
if ($down > 0){
$con->query("UPDATE `package_download` SET `downs` = `downs`+'1' WHERE `user_id` = '".$user['id']."' AND `package_id` = '".$b['id']."'");
}else{
$con->query("INSERT INTO `package_download` (`id`, `user_id`, `downs`, `package_id`) VALUES (NULL, '".$user['id']."', '1', '".$b['id']."')");
}
}
